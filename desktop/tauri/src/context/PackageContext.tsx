...
export function PackageProvider({ children }: { children: ReactNode }) {
  ...
  const [osUserName, setOsUserName] = useState("user");

  useEffect(() => {
    void invoke<string>("get_os_user_name")
      .then((n) => {
        if (n?.trim()) setOsUserName(n.trim());
      })
      .catch(() => undefined);
  }, []);

  const userName = user?.displayName?.trim() || user?.login || osUserName;
