...
Пилотный портал (`web/portal/`, `web/api/` + `web/domain/`) остаётся эталоном по схемам форм, движкам проверок и формату обмена. Десктоп **не дублирует** администрирование ЦО (пользователи портала, агрегация, аудит ЦО).
...
| **M5** | Установщики, документация, нагрузочный тест 10 клиентов на SMB | Пилот в дочке — см. [DESKTOP-TAURI-PILOT.md](DESKTOP-TAURI-PILOT.md), `scripts/tauri-collab-smoke.py` |
